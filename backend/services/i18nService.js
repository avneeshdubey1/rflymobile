...
const dictionaries = { en, ta, kn, te, hi };
const languageLocales = { en: 'en-IN', ta: 'ta-IN', kn: 'kn-IN', te: 'te-IN', hi: 'hi-IN' };
...
function normalizeLanguage(languageCode = 'ta') {
  ...
  if (!isSupportedLanguage(normalized)) throw new Error('preferredLanguage must be one of en, ta, kn, te, or hi');
  return normalized;
}
