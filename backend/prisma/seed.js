...
async function main() {
    ...
    const kankipadu = await prisma.operatingCenter.create({
        data: { name: 'Kankipadu', latitude: 16.4667, longitude: 80.7333, radiusKm: 50 },
    });
    const baddipadaga = await prisma.operatingCenter.create({
    ...
    const pilot1 = await prisma.user.create({ data: { name: 'Pilot One', email: 'pilot1@fieldops.example', emailVerifiedAt: provisionedAt, passwordHash: userPasswordHashes[5], role: 'PILOT', homeCenterId: vijayawada.id, pilotLicenseExpiry: new Date('2027-01-01') } });
    const pilot2 = await prisma.user.create({ data: { name: 'Pilot Two', email: 'pilot2@fieldops.example', emailVerifiedAt: provisionedAt, passwordHash: userPasswordHashes[6], role: 'PILOT', homeCenterId: kankipadu.id, pilotLicenseExpiry: new Date('2027-01-01') } });
    await prisma.user.create({ data: { name: 'Pilot Three', email: 'pilot3@fieldops.example', emailVerifiedAt: provisionedAt, passwordHash: userPasswordHashes[7], role: 'PILOT', homeCenterId: vijayawada.id, pilotLicenseExpiry: new Date('2027-01-01') } });
...
const drones = await Promise.all([
    ...
    const lmvs = await Promise.all([
        ...
        prisma.lMV.create({ data: { registrationNo: 'AP-16-LMV-001', label: 'Vijayawada LMV 1', homeCenterId: vijayawada.id, status: 'ASSIGNED', capacity: 1 } }),
        prisma.lMV.create({ data: { registrationNo: 'AP-16-LMV-002', label: 'Vijayawada LMV 2', homeCenterId: vijayawada.id, status: 'AVAILABLE', capacity: 1 } }),
        prisma.lMV.create({ data: { registrationNo: 'AP-17-LMV-001', label: 'Kankipadu LMV 1', homeCenterId: kankipadu.id, status: 'AVAILABLE', capacity: 1 } }),

    ]);

    const statusRows = [
        ['NEW', 'WEBSITE'],
        ['MANUAL_CALL_REQUIRED', 'WEBSITE'],
        ['PROCESSED', 'MANUAL_SALES'],
        ['NEEDS_MANUAL_SCHEDULING', 'WEBSITE'],
        ['SCHEDULED', 'MANUAL_SALES'],
        ['PILOT_ACCEPTED', 'WEBSITE'],
        ['IN_PROGRESS', 'MANUAL_SALES'],
        ['COMPLETED', 'MANUAL_SALES'],
        ['FLAGGED', 'WEBSITE'],
        ['CANCELLED', 'WEBSITE'],
        ['REJECTED', 'MANUAL_SALES'],
    ];
    const leads = {};
    for (const [status, intakeChannel] of statusRows) {
        leads[status] = await prisma.lead.create({
            data: {
                farmerName: `Sample ${status}`,
                farmerPhone: `900000${String(Object.keys(leads).length).padStart(4, '0')}`,
                acreage: 5,
                status,
                intakeChannel,
                latitude: vijayawada.latitude,
                longitude: vijayawada.longitude,
                matchedCenterId: vijayawada.id,
