...
**Plan status:** This remains the future-delivery contract. The additive
master/history schema and controlled farmer importer are implemented and have
completed staging reconciliation. Production import, drone-master onboarding,
provider delivery, billing evidence, and final production acceptance remain
gated. Use [CURRENT_ENGINEERING_STATE.md](CURRENT_ENGINEERING_STATE.md) for the
implemented state; do not infer it from older milestones in this plan.
...
The codebase is not yet at the complete target. Strict intake, lightweight LMV
scheduling, Sales-assisted customer lookup, portal isolation, application-owned
OTP, Firebase removal, Pilot-centre enforcement, two-person crew/schema support,
master/history evolution, and the guarded farmer import pipeline have regression
or staging evidence. The application still lacks the complete Billing Case,
flight-evidence store, object-storage integration, dedicated evidence worker,
and live WhatsApp/SMS adapter. Production import and final acceptance are not
complete merely because staging reconciliation passed.
...
- The approved target operational unit is one primary Pilot, one Copilot, one drone, and one LMV. The development branch implements both crew references and shared mission access; production migration evidence remains pending.
- One operational unit may receive multiple jobs per day with an explicit daily sequence. The development branch implements manual sequencing and serialized non-overlapping mission starts. Route optimization remains deferred.
- Every Pilot requires an active operating centre. Admin and Fleet may change it only when the Pilot has no active assignment; scheduling and rescheduling revalidate active PILOT role and same-centre eligibility.
- Operational direct chat follows `Admin > Fleet Manager > Sales > Pilot`. Only higher-to-lower initiation is allowed, the supervisor sends first, and only Admin may close sessions.
...
Implement the challenge before choosing a provider. The first delivery boundary includes a disabled production-safe adapter, a deterministic test adapter, a local developer CLI/console adapter, and one validated selector such as `OTP_DELIVERY_PROVIDER`. The CLI/console adapter exists only so developers can validate the OTP challenge before WhatsApp/SMS delivery is available; it may show the OTP in a local terminal window in explicit development/test mode only and must fail closed in production. The adapter contract sends an authentication OTP, validates a callback signature, normalizes a delivery event, and declares channel capabilities. Provider-specific values—API token, sender, template ID, base URL, and callback secret—belong only in deployment secrets or mounted secret files.

When the client confirms Meta, another WhatsApp provider, or an SMS provider, add a small adapter that maps this contract to that provider. The OTP database model, code verification, sessions, UI flow, rate limits, and audit policy do not change. Until then, no real provider is activated and no credential is requested or placed in an environment file. Firebase removal is immediate after internal OTP browser/recovery cutover and does not wait for real WhatsApp/SMS activation; local validation uses disabled, deterministic test, and CLI adapters.
...
- Remove Firebase browser/server packages, configuration, deployment references, token-proof services, and legacy tests immediately after the internal OTP browser/recovery cutover passes local security regression. External service-account revocation remains an owner evidence task. Real WhatsApp/SMS provider activation remains held behind client/provider confirmation.
...
- Treat the client handover as an immediate migration priority, beginning with approved farmer/customer data and then approved core masters.
- Implement the first import as a controlled one-time script/workflow, not a standing Admin upload feature.
...
| Merge to staging | Immutable images identified by source revision/digest and automatic deployment to an isolated, office-internal staging stack. |
| Staging to main promotion | Release-owner approval, pinned revision/digest, verified backup, controlled migration, production health/workflow verification, monitoring confirmation, and rollback window. |
