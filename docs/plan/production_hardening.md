...
- Until a dedicated staging host is approved, the office server may run a second internal-only staging Compose stack. It must have a different project name, port, secret set, database volume, and non-production dataset from the production stack.
...
- [x] Implement strict transient geofence processing and 30-day Declined Enquiry purge; local regression proves no rejected location/distance/acreage persists.
- [x] Remove appeal/transport-fee/Google Form/Bhumeet operational paths through tested migrations and route retirement on the fresh-handover path. Populated legacy-data archival still requires explicit approval and staging evidence.
...
- [x] Build and test the local provider-neutral OTP delivery port, disabled production-safe adapter, deterministic test adapter, local developer CLI/console adapter, validated `OTP_DELIVERY_PROVIDER` selector, and secret-file/environment configuration boundary. Production worker/webhook/fallback evidence remains open. The CLI adapter may display OTPs only in explicit local/test mode, must fail closed in production, and must never expose OTPs through browser responses, AuditLog, persistent logs, fixtures, reports, or documentation. An API key cannot enable an adapter that has not been implemented and reviewed.
...
| July 28, 2026 | GitHub main/dev-ing self-hosted CI/CD path | Git remote review, branch setup, workflow/script review | Added `dev-ing` CI trigger and an on-prem self-hosted deploy workflow that deploys only after CI passes on `main` or after authorized manual dispatch. The workflow requires a GitHub runner on the office server with `self-hosted`, `linux`, and `rfly-onprem` labels. It deploys through the existing Compose files and records non-secret deployment evidence. |
| July 29, 2026 | Demo stabilization and incomplete-feature retirement | Backend integration suite, real Edge browser audit, frontend lint/build, disposable production Compose build/migration/health checks | Passed locally after the final cleanup: backend 95/95, browser 33/33, frontend lint/build, and isolated production stack migration plus edge/API health. Added mandatory Pilot centre management and reassignment validation; enforced `Admin > Fleet > Sales > Pilot` chat initiation with Admin-only closure; retired immediate payment, public Business self-registration, fake drone inquiry/maintenance-approval surfaces, lead-chat leftovers, and dead legacy assignment endpoints. Evidence-backed billing, live delivery providers, staging operations, and client acceptance remain open. |
| August 31, 2026 | Pilot Copilot-deadline and B2C cash-report local candidate | Backend 176/176; Pilot 22/22 plus typecheck; frontend lint/build; 36-migration/66-check clean and populated replay; local API/database health | Local gates pass. Future service windows preserve Copilot-selection time; direct-route back navigation is fail-safe; completed B2C cash handovers are exact, idempotent, audited, and Admin-visible. This is not price approval, invoice issuance, settlement reconciliation, UPI enablement, staging acceptance, or production approval. |
| August 31, 2026 | Fleet preference, calendar, realtime roster, and maintenance-lifecycle local candidate | Full isolated backend run 176/177, followed by the sole affected direct-controller file 3/3 after its realtime-helper fix; new asset-lifecycle regression passed; frontend lint/build; targeted calendar browser 2/2; 37-migration/66-check clean and populated replay; local API/database health | Shareable Pilot asset preferences no longer masquerade as reservations; Assignment exclusivity remains enforced. Admin/Fleet maintenance uses structured reasons and exposes attributed request/lifecycle history. Calendar navigation and popout are local candidates. A fresh full 177/177 run, human visual/functional acceptance, staging evidence, and production approval remain open. |
| September 1, 2026 | Consolidated Release 1 and browser Pilot crew candidate | Backend 179/179; Pilot TypeScript, 22/22 Jest and Expo Doctor 21/21; frontend lint/build; focused real-browser 4/4; 37-migration/66-check clean and populated replay | Local automated gates pass for the consolidated candidate. Browser and Android Pilot clients share the serialized Primary-Pilot Copilot contract. Human role/device acceptance, staging deployment evidence, and production health/rollback observation remain open. |
