...
| PORTAL-01 | Record approved Farmer linking and Business membership rules, including the dev-only OTP validation path. | G-03 | Stop if the client has not chosen the portal-linking rule. The documented OTP validation path must be a local CLI/console adapter only, unavailable in production, and must not replace WhatsApp/SMS provider delivery later. |
...
| OTP-08 | Add disabled production-safe, deterministic test, and local developer CLI/console delivery adapters. | OTP-04 | Default configuration cannot send a real message. The CLI adapter prints the OTP only in explicit local/test mode, never production, never browser responses, never AuditLog, and never persistent logs. |
