...
| Portal enrolment | Local portal reads require explicit server-side linkage. Sales/Admin can now enable Farmer portal access for a staff-confirmed Customer by creating or linking an active FARMER user, without issuing a Farmer session or impersonating the customer. Farmer self-registration is retired; external Farmer portal access requires the linked Farmer account and internal OTP login. | Confirm farmer-support wording, client operating policy for when staff may enable access, and staging privacy review before external self-service portal launch. | Operations/privacy | Farmer portal launch |
