...
| Portal enrolment | Local portal reads require explicit server-side linkage. Sales may staff-confirm a customer/Farmer record during a phone call without OTP for internal request handling, but that does not grant a Sales user a Farmer session. Farmer self-registration is retired; external Farmer portal access requires an active linked Farmer account and internal OTP login. | Confirm invitation/support wording and build the Farmer phone-link/invitation workflow before external self-service portal launch. | Operations/privacy | Farmer portal launch |
