...
| WhatsApp, SMS, and email | No live provider is selected or approved. Firebase cannot deliver WhatsApp OTP. The target starts with a provider-neutral adapter plus disabled/test adapters and a local developer CLI/console adapter that may display OTPs only in explicit development/test mode. The CLI adapter is validation scaffolding, not a messenger, and must be unavailable in production. Meta is only a researched candidate. | Client provider decision, account/sender, eligibility, approved templates, opt-in wording, rate-card evidence, budget owner, signed callbacks, sandbox results, and an India-capable SMS provider where required. | Business/provider owner | Live communication and recovery |
| Phone verification and Firebase retirement | Current Farmer proof and Business phone recovery still rely on Firebase; there is no generic application-owned phone-verification challenge or durable delivery worker yet. Firebase has no Firestore/Storage/FCM data role in this repository. OTP-01 policy is approved for implementation: 6-digit code, 5-minute expiry, 5 attempts, 30-second resend cooldown, one active challenge per recipient/purpose, short metadata retention, no raw OTP retention outside local/test CLI display, and immediate Firebase removal after internal OTP cutover. | Meta and SMS sandbox evidence, provider cutover owner/window, and external Firebase service-account revocation evidence after repository removal. | Security/provider owner | Farmer/Business phone verification and production approval |
| Portal enrolment | Local portal reads now require explicit server-side linkage. Sales may staff-confirm a customer/Farmer record during a phone call without OTP for internal request handling, but that does not grant external portal access until the approved link/invitation and OTP policy is implemented. Firebase-era Farmer self-registration remains legacy until OTP cutover. | Confirm invitation/support wording and complete OTP provider-independent challenge before external portal launch. | Operations/privacy | Farmer portal launch |
