...
| Customer and organization linkage | CX-01 map is recorded. Current records are lead-centric: Leads store farmer name/phone directly, Farmer/Business are User roles, and Sales manual intake creates MANUAL_SALES Leads without a Customer table. The approved replacement seam is Sales-only canonical-phone lookup/create plus a staff-scoped Farmer Service View; Sales must not receive a real Farmer session or bypass service-area checks. | Business organization/membership rules, customer-data ownership, and approved portal-linking/invitation policy. | Operations/privacy | Sales-assisted customer workflow and portal launch |
