...
| P00-01 to P00-02 | PASS | Bounded assignment list/detail DTOs expose only assigned work; Primary Pilot eligible-Copilot selection and reasoned Fleet/Admin override reuse the serialized crew domain service. Phase 32 passes 6/6 including availability and foreground-location cases. |
...
| P10-04 | LOCAL PASS / PRODUCTION GATED | Foreground-only latest-location submission now validates assigned crew, availability, mission state, bounds, accuracy, timestamp and cadence; Admin/Fleet viewing and coordinate-free audit tests pass. Background location and route history remain absent. Written production notice, retention and privacy approval is still required before activation. |
...
| Q00-01 | PASS WITH PRODUCTION LOCATION GATE OPEN | Complete local acceptance passes: backend 160/160; Prisma clean/populated replay applies 29 migrations with 61 validated checks and preserves legacy counts; frontend lint/build pass; Pilot mobile typecheck, 18/18 tests and Expo Doctor 21/21 pass. Earlier isolated Compose health and non-root/no-internal-port assertions remain valid, but the new mobile location capability still requires physical-device/privacy acceptance before production activation. |
