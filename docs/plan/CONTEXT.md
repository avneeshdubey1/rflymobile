...
## Fleet reality: two-person crew, drone, and LMV

The client has clarified that one operational unit contains one LMV, one drone, a primary Pilot, and a Copilot. Both crew members share driving, surveying, drone operation, battery changes, and loading/unloading duties. A job therefore needs four coordinated resources:

1. a qualified, available primary Pilot;
2. a qualified, available Copilot;
3. an available, compliant drone; and
4. an available, compliant LMV from the appropriate operating centre.

One crew may perform multiple jobs in one day. The schedule must preserve required service times and an explicit daily job order while preventing either crew member, the drone, or the LMV from being double-booked. Automated route optimization is not assumed until the client supplies its planning rules.

The development branch now stores a primary Pilot and Copilot, exposes ordered daily jobs, and blocks concurrent starts across either crew member, the drone, or the LMV. Production migration and browser/database acceptance evidence remain required before this is treated as live.

## Immediate migration priority

The immediate operational priority is staff-assisted farmer/customer data entry and migration from client Excel sheets and Zoho CRM exports. Farmer login, billing, Business commission, and automated route optimization are deferred from this short migration window. The import remains a controlled one-time procedure rather than a routine Admin feature.
