...
Phase 1 source now enforces strict service-area intake and retires active Google Form, appeal, transport-fee, and Bhumeet mock paths. Lightweight LMV scheduling is implemented locally: a scheduled job reserves a pilot, drone, and LMV, and releases the LMV when the mission is completed or decommissioned. Sales-assisted customer intake is also implemented locally: Sales/Admin can search or create a customer, open an audited staff-scoped service view, and raise a customer-linked request without OTP or Farmer-session impersonation while preserving public/Farmer request flows. Read-only portal isolation is implemented locally: Farmer reads are linked through Customer records and Business reads require explicit organization membership and linked work. The strict-intake forward migration intentionally stops on populated legacy data until an approved backup/archival procedure is evidenced. Immediate manual-acreage payment creation and live WhatsApp/SMS provider activation remain later delivery work; Farmer portal authentication and Business recovery use the application-owned OTP challenge.
